    ...
    >
      {slides.map((slide) => (
        <SwiperSlide key={slide.image_url}>
         ...
         <p className="absolute top-1/3 md:top-11/16 lg:top-11/25 hidden md:block  left-1/2 transform -translate-x-1/2  -translate-y-1/2 text-white text-base md:text-lg opacity-90 text-center ">{slide.description}</p>
         </div>
        </SwiperSlide>
      ))}
    </Swiper>
